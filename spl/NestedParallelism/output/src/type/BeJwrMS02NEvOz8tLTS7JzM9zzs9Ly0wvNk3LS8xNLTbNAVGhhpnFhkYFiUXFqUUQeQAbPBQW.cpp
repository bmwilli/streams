...
#include "BeJwrMS02NEvOz8tLTS7JzM9zzs9Ly0wvNk3LS8xNLTbNAVGhhpnFhkYFiUXFqUUQeQAbPBQW.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<rstring connectionConfig,rstring fname,rstring lname,uint64 i,rstring parserConfig>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    {
        std::string s("connectionConfig");
        TypeOffset t(MY_OFFSETOF(connectionConfig_, tm), 
    ...
    {
        std::string s("parserConfig");
        TypeOffset t(MY_OFFSETOF(parserConfig_, tm), 
                     Meta::Type::typeOf<SPL::rstring >(), 
                     &typeid(SPL::rstring));
        addMapping(*tm, t, s, 4);
    }
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (s != "connectionConfig") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, connectionConfig_);
   else
     istr >> connectionConfig_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
   if (c != ',') { istr.setstate(std::ios_base::failbit); return; }
   ...
   if (!readAttributeIdentifier(istr, s)) { return; }
   if (s != "parserConfig") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, parserConfig_);
   else
     istr >> parserConfig_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, connectionConfig_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
   if (c != ',') { istr.setstate(std::ios_base::failbit); return; }
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, parserConfig_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
{
    ostr << '{'
         << "connectionConfig=" << get_connectionConfig()  << ","
         << "fname=" << get_fname()  << ","
         << "lname=" << get_lname()  << ","
         << "i=" << get_i()  << ","
         << "parserConfig=" << get_parserConfig()  
         << '}';
}
...
{
    ostr << '{';
    SPL::serializeWithPrecision(ostr << "connectionConfig=", get_connectionConfig()) << ",";
    ...
    SPL::serializeWithPrecision(ostr << "parserConfig=", get_parserConfig()) ;
    ostr << '}';
}

SELF& SELF::clear()
{
    SPL::rstring().swap(get_connectionConfig());
    SPL::rstring().swap(get_fname());
    SPL::rstring().swap(get_lname());
    get_i() = 0;
    SPL::rstring().swap(get_parserConfig());
